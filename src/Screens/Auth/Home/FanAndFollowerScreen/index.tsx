...
export default function FanAndFollowerScreen({ navigation, route }: any) {
  ...
  const fetchFollowers = async (userId: number) => {
    try {
      ...
      if (screenName === 'Fans') {
        response = await axios.get(`${envVar.API_URL}user-fans/${user.id}`);
        result = response?.data?.fans || [];
      } else if (screenName === 'Following') {
        response = await axios.get(
          `${envVar.API_URL}user-following/${user.id}`,
        );
        result = response?.data?.following || [];
      } else if (screenName === 'Friends') {
        response = await axios.get(`${envVar.API_URL}user-friends/${user.id}`);
        result = response?.data?.following || [];
      }
      ...
      {loading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[
            styles.listContent,
            filteredFollowers.length === 0 && { flex: 1 }, // Add this to make empty state centered
          ]}
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>
                {search
                  ? 'No results found'
                  : `No ${screenName.toLowerCase()} to show`}
              </Text>
            </View>
          }
        />
