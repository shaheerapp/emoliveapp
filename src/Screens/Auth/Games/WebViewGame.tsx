...
const WebViewGame: React.FC<Props> = ({ navigation, route }) => {
  ...
  const triggerWalletUpdate = () => {
    ...
    const jsCode = `window.walletUpdate && window.walletUpdate(${JSON.stringify(
      walletUpdatePayload,
    )});`;
    ...
    try {
      ...
      if (jsCallback.includes('getConfig')) {
        const configData = {
          ...
          appId: 8835275887, // your actual appId
          userId: user?.id || 'guest_001',
          ...
          gameConfig: {
            sceneMode: 0,
            currencyIcon: require('../../../assets/icons/diamond.png'),
          },
          gsp: 301,
        };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {showWebView && (
        <WebView
          ...
          onMessage={(event: any) => handlemessage(event)}
