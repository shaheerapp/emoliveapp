...
interface Props {
    onPlayGame: (url: string, user: any, code: string) => void;
}

const BottomGames: React.FC<Props> = ({ onPlayGame }) => {
    ...
    const handlePlay = async (game: any) => {
        try {
            ...
            onPlayGame(game.download_url, user, code);
        } catch (err) {
    ...
    useEffect(() => {
        const loadGames = async () => {
            try {
                const data = await fetchGameList(8835275887, 'emolive');
                setGames(data);
            } catch (error) {
